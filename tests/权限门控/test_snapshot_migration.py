...
def test_from_dict_chains_v1_all_the_way_to_v4_intermediate():
...
def test_from_dict_migrates_v2_to_v4_intermediate():
    payload = _v1_payload()
    payload["version"] = 2
...
def test_from_dict_migrates_v3_to_v4_preserves_names_intermediate():
    payload = _v1_payload()
    payload["version"] = 3
    payload["extraction_watermark"] = 5
    payload["runs_since_extraction"] = 3
...
def test_migrate_v4_to_v5_sets_pending_approval_defaults():
    from harness.snapshot.models import migrate_v4_to_v5
    v4_payload = migrate_v3_to_v4(migrate_v2_to_v3(migrate_v1_to_v2(_v1_payload())))
    migrated = migrate_v4_to_v5(v4_payload)
    assert migrated["version"] == 5
    assert migrated["pending_approval_id"] is None
    assert migrated["pending_tool_call_id"] is None


def test_from_dict_chains_v1_all_the_way_to_v5():
    snap = RunSnapshot.from_dict(_v1_payload())
    assert snap.version == 5
...
def test_from_dict_accepts_current_v5_directly():
    payload = _v1_payload()
    payload["version"] = 5
    payload["extraction_boundary_hid"] = None
    payload["runs_since_extraction"] = 0
    payload["surfaced_memories"] = []
    payload["pending_approval_id"] = "appr-1"
    payload["pending_tool_call_id"] = "call_5"
    snap = RunSnapshot.from_dict(payload)
    assert snap.pending_approval_id == "appr-1"
    assert snap.pending_tool_call_id == "call_5"
