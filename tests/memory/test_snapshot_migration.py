...
from harness.snapshot.models import RunSnapshot, migrate_v1_to_v2, migrate_v2_to_v3
...
def test_from_dict_chains_v1_all_the_way_to_v3():
    """v1 快照直接 from_dict 应该一路链式迁移到当前版本(v3),
    不需要调用方手动分两步迁移。"""
    snap = RunSnapshot.from_dict(_v1_payload())
    assert snap.version == 3
    assert snap.extraction_watermark is None
    assert snap.runs_since_extraction == 0
    assert snap.surfaced_memories == []
    assert snap.session_id == "s1"


def test_from_dict_migrates_v2_to_v3():
    payload = _v1_payload()
    payload["version"] = 2
    ...
    assert snap.version == 3
    assert snap.extraction_watermark == 5
...
def test_from_dict_accepts_current_v3_directly():
    payload = _v1_payload()
    payload["version"] = 3
    payload["extraction_watermark"] = 5
    payload["runs_since_extraction"] = 3
    payload["surfaced_memories"] = ["user_pref_a"]
    snap = RunSnapshot.from_dict(payload)
    assert snap.extraction_watermark == 5
    assert snap.runs_since_extraction == 3
    assert snap.surfaced_memories == ["user_pref_a"]


def test_round_trip_json():
    snap = RunSnapshot(
        session_id="s2", task="t", trace_id="tr", status="completed",
        rounds=1, tool_calls_used=0, extraction_watermark=3, runs_since_extraction=2,
        surfaced_memories=["a", "b"],
    )
    import json
    payload = json.loads(snap.to_json())
    restored = RunSnapshot.from_dict(payload)
    assert restored.extraction_watermark == 3
    assert restored.runs_since_extraction == 2
    assert restored.surfaced_memories == ["a", "b"]