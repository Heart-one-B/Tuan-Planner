...
def test_loop_config_is_frozen():
    """用 setattr 而不是 `cfg.max_terminal_nudges = 99` 直接赋值,
    原因和测试逻辑无关,是工具层面的:PyCharm 的 dataclass 静态检查
    看到 frozen dataclass 上的直接赋值就会标红("object attribute is
    read-only")。它说得没错——那行确实违反语言规则——但它不知道
    这**正是本测试要验证的东西**,整条测试的目的就是确认这个非法赋值
    会抛异常。

    setattr 是动态调用,静态分析看不穿,红线自然消失;而运行时它走的是
    完全同一条 __setattr__ 路径(frozen dataclass 就是靠重写
    __setattr__ 实现的),抛出的异常一字不差,断言强度没有任何削弱。

    没有选择 `# noinspection PyDataclass`:那是 PyCharm 专有注释,
    换个编辑器/linter 就失效,而且它会压掉整块区域的检查——包括可能
    真正有价值的告警。让红线消失的正确方式是让代码本身不触发误报,
    不是把告警关掉。
    """
    cfg = _make_cfg(FakeLLMClient([]), ToolExecutor())
    with pytest.raises(dataclasses.FrozenInstanceError):
        setattr(cfg, "max_terminal_nudges", 99)
...
async def test_run_query_loop_itself_propagates_exceptions_without_sealing_span(
    ...
    【Phase 4 之后】原来这里断言的是 `trace_id in tracer._active`,
    伸手进模块级私有字典。现在改用 conftest 注入的隔离 Tracer 实例
    + 公开的 active_trace_ids()——断言的语义完全没变("这条 trace
    还没被结束"),但不再依赖任何全局状态,也不再依赖私有属性名。
    """
    llm = RaisingLLMClient(RuntimeError("boom"))
    executor = ToolExecutor()
    run_ctx = _run_ctx()
    ...
    assert run_ctx.span.trace_id in isolated_tracer.active_trace_ids()
